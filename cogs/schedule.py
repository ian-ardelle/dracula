        ...
        for guild_id in db.get_guild_list():
            ...
            if cur_date != last_date:
                ...
                for player in player_list:
                    ...
                    if current_bp >= 0:
                        db.c.execute("UPDATE Characters SET bp = %d WHERE id = %d", (current_bp, player.get("id")))
                    current_wp = player.get("wp")
                    current_wp_max = player.get("wp_max")
                    if current_wp < current_wp_max:
                        current_wp += 1
                        db.c.execute("UPDATE BnW SET wp = %d WHERE player_id = %d", (current_wp, player.get("id")))
                    ...
                    if not old_upkeep:
                        ...
                        old_upkeep = ctime
                    if old_upkeep < ctime and player.get("upkeep") > 0:
                        if player.get("bp") > 0:
                            ...
                            db.c.execute("UPDATE Characters SET upkeep_date = %s, bp = %d WHERE id = %d",
                                         (upkeep_datetime.strftime("%Y:%m:%d:%H:%M:%S"), new_bp, player.get("id")))
                    if player.get("bp") == 0:
                        if player.get("alert_flag") == 0:
                            db.c.execute("UPDATE Characters SET alert_flag = 1 WHERE id = %d", player.get("id"))
                            try:
                    ...
                    if player.get("alert_flag") == 1:
                        if player.get("bp") > 0:
                            db.c.execute("UPDATE Characters SET alert_flag = 0 WHERE id = %d", player.get("id"))
